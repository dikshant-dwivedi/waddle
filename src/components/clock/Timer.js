...
export class Timer extends Component {
    ...
    {
        if (this.props.timerControl === "enabled" && this.props.timerControl !== prevProps.timerControl)
        {
            console.log("componentdidupdate: enabled")
            this.setState({
                disabledAll: false
            })
        }
        ...
        {
            if (this.props.timerControl === "disabled" && this.props.timerControl !== prevProps.timerControl)
            {
                console.log("componentdidupdate: disabled")
                this.setState({
                    disabledAll: true 
                })
            }
            else if(this.props.timerControl === "play" && !this.state.isPlaying && this.props.timerControl !== prevProps.timerControl)
            {
                console.log('componentdidupdate: play')
                this.setState({
                    workMinute: this.props.timerValues.workMinute,
                    breakMinute: this.props.timerValues.breakMinute,
                    timerSecond: this.props.timerValues.timerSecond,
                    isFlipped: this.props.timerValues.isFlipped,
                    isWorkSession: this.props.timerValues.isWorkSession
                })
                this.play()
            }
            else if(this.props.timerControl === "stop" && this.state.isPlaying)
            {
                console.log('componentdidupdate: stop')
                this.stop()
            }
            else if(this.props.timerControl === "reset" && !this.state.isPlaying && this.props.timerControl !== prevProps.timerControl)
            {
    ...
    playSocket() {
        if (this.props.roomCode && this.props.socket) {
            ...
            this.props.socket.emit('play',timerValues ,
             () => { console.log("a play callback was sent"); })
        }
        else {
            this.play()
        }
        ...
        if (isVisible === false) {
            if (this.state.isWorkSession && this.state.isPlaying) {
                const timerId = setTimeout(() => {
                    if(this.props.socket && this.props.roomCode)
                    {
                        this.props.setTimerControl("stop")
                        this.props.setTimerControl("reset")
                    }
                    else
                    {
                    this.stop()
    ...
    render() {
        return (
            <section className="pomodoro-clock-section">
                <TimerInterrupted clearModal={this.clearModal} show={this.state.show} lostFocus = {this.props.lostFocus}/>
        ...
        <div className="timer-work-session">
            ...
            <header className="headings-container">
                ...
                <Switch onChange={handleSessionChange}
                    ...
                    onColor="#000"
                    offColor="#000"
                    disabled={props.isPlaying || props.disabledAll}
                    handleDiameter={18}
                    height={24} />
            ...
            <Button variant="dark"
                ...
                disabled={props.resetDisabled || props.disabledAll}
